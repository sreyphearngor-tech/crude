...
<div class="container mx-auto px-4 py-10 md:px-12 lg:px-24">
    ...
    <nav class="text-sm text-gray-500 mb-8">
        ...
        <a href="{{ route('category.products', $product->category_id) }}" class="hover:text-red-500">{{ $product->category->name }}</a> /
    ...
    <div class="flex flex-col lg:flex-row gap-12">
        ...
        <div class="w-full lg:w-3/5 flex flex-col-reverse md:flex-row gap-4">
                ...
                @foreach(['image', 'image2', 'image3', 'image4'] as $imgField)
                    @if($product->$imgField)
                        <div class="w-20 h-20 md:w-24 md:h-24 bg-gray-100 rounded-md p-2 cursor-pointer border hover:border-red-500 transition-all thumbnail-btn"
                             onclick="changeMainImage('{{ asset('storage/' . $product->$imgField) }}')">
                            <img src="{{ asset('storage/' . $product->$imgField) }}" class="w-full h-full object-contain">
                        </div>
                    @endif
                @endforeach
            </div>
            ...
            <div class="flex-1 bg-gray-100 rounded-lg p-10 flex items-center justify-center min-h-[400px]">
                <img id="mainImage" src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="max-w-full max-h-[500px] object-contain transition-all duration-300">
            </div>
        </div>
        ...
        <div class="w-full lg:w-2/5 space-y-6">
            ...
            <div class="flex items-center gap-4">
                ...
                <span class="text-green-500 text-sm font-medium border-l pl-4">In Stock ({{ $product->qty }})</span>
            </div>
            ...
            <!-- Quantity & Buy Action -->
            <form action="{{ route('cart.store') }}" method="POST" class="flex flex-col sm:flex-row gap-4 pt-4">
                @csrf
                ...
                <div class="flex border rounded-md overflow-hidden w-fit">
                    ...
                    <input type="number" name="quantity" id="prod_qty" value="1" min="1" max="{{ $product->qty }}" class="w-16 text-center border-x focus:outline-none appearance-none">
                    <button type="button" onclick="updateQty(1)" class="px-4 py-2 hover:bg-red-500 hover:text-white transition-colors">+</button>
                </div>
